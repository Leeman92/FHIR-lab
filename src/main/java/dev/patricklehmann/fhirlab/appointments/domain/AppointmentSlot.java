...
public class AppointmentSlot {
    ...
    public AppointmentSlot(Instant startTime, Instant endTime) {
        ...
        if (!endTime.isAfter(startTime)) {
            throw new IllegalArgumentException("The appointments end time must be after the start time");
        }

        if (isNotOnFiveMinuteBoundary(startTime)) {
            throw new IllegalArgumentException("The start time must fall on a full five-minute boundary");
        }

        if (isNotOnFiveMinuteBoundary(endTime)) {
            throw new IllegalArgumentException("The end time must fall on a full five-minute boundary");
        }
        ...
        if (appointmentDuration.toMinutes() < 15 || appointmentDuration.toMinutes() > 180) {
            throw new IllegalArgumentException("The duration of the appointment must be between 15 Minutes and 3 hours");
        }
